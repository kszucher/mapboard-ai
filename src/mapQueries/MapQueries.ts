...
export const getXR = (m: M): R =>
  mR(m)
    .filter(ri => ri.selected)
    .reduce((a, b) => (a.selected > b.selected ? a : b), <R>{});

export const getAXR = (m: M): R[] =>
  mR(m)
    .filter(ri => ri.selected)
    .sort(sortPath);
...
export const isAXL = (m: M): boolean =>
  hasTrues(
    mT(m)
      .filter(ti => ti.selected)
      .map(el => isL(el.path))
  );

export const isAXR = (m: M): boolean =>
  hasTrues(
    mT(m)
      .filter(ti => ti.selected)
      .map(el => isR(el.path))
