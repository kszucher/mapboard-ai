...
export const getLinearLinePath = ({ x1, x2, y1, y2 }: {
  x1: number,
  x2: number,
  y1: number,
  y2: number
...
const getCoordinatesForSide = (node: R, side: Side): { x: number, y: number, cx: number, cy: number } => {
  ...
  switch (side) {
    ...
    case Side.L:
      return { x: nodeStartX, y: nodeStartY + selfH / 2, cx: nodeStartX - offset, cy: nodeStartY + selfH / 2 }
    case Side.T:
      return { x: nodeStartX + selfW / 2, y: nodeStartY, cx: nodeStartX + selfW / 2, cy: nodeStartY - offset }
    case Side.B:
      return {
