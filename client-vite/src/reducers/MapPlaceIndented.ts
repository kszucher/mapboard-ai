import {isC, isCS, isCSC, isR, isRS, isRSC, isS, isSS, isSSC, isSU, mT} from "../queries/MapQueries.ts"
  ...
  mt.forEach(ti => {
    switch (true) {
      ...
      case isS(ti.path): {
        ...
        const elapsed = mt.filter(nt => isSU(ti.path, nt.path)).map(ti => ti.maxH).reduce((a, b) => a + b, 0) + i * S_SPACING * +Boolean(si1.tco2.length)
        if (isRS(ti.path)) {
