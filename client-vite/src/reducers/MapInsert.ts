...
import {genHash, getTableIndices, IS_TESTING} from "../utils/Utils"
...
export const insertS = (m: M, ip: PS, attributes: object) => {
  ...
  m.push({
    selected: 1,
    nodeId: IS_TESTING ? '_' + ip.join('') : 'node' + genHash(8),
...
export const insertCRD = (m: M) => {
  ...
  m.push(...getXAC(m).map(ci => ({
    nodeId: IS_TESTING ? '_' + ci.path.with(crIndex, crValue + 1).join('') : 'node' + genHash(8),
...
export const insertCRU = (m: M) => {
  ...
  m.push(...getXAC(m).map(ci => ({
    nodeId: IS_TESTING ? '_' + ci.path.with(crIndex, crValue).join('') : 'node' + genHash(8),
...
export const insertCCR = (m: M) => {
  ...
  m.push(...getXAC(m).map(ci => ({
    nodeId: IS_TESTING ? '_' + ci.path.with(ccIndex, ccValue + 1).join('') : 'node' + genHash(8),
...
export const insertCCL = (m: M) => {
  ...
  m.push(...getXAC(m).map(ci => ({
    nodeId: IS_TESTING ? '_' + ci.path.with(ccIndex, ccValue).join('') : 'node' + genHash(8),
...
export const insertSCRD = (m: M) => {
  m.push(...Array.from({length: getXS(m).colCount}, (_, i) =>({
    nodeId: IS_TESTING ? '_' + [...getXS(m).path, 'c', getXS(m).rowCount, i].join('') : 'node' + genHash(8),
...
export const insertSCRU = (m: M) => {
  ...
  m.push(...Array.from({length: getXS(m).colCount}, (_, i) =>({
    nodeId: IS_TESTING ? '_' + [...getXS(m).path, 'c', 0, i].join('') : 'node' + genHash(8),
...
export const insertSCCR = (m: M) => {
  m.push(...Array.from({length: getXS(m).rowCount}, (_, i) =>({
    nodeId: IS_TESTING ? '_' + [...getXS(m).path, 'c', i, getXS(m).colCount].join('') : 'node' + genHash(8),
...
export const insertSCCL = (m: M) => {
  ...
  m.push(...Array.from({length: getXS(m).rowCount}, (_, i) =>({
    nodeId: IS_TESTING ? '_' + [...getXS(m).path, 'c', i, 0].join('') : 'node' + genHash(8),
...
export const insertTable = (m: M, ip: PS, payload: {rowLen: number, colLen: number}) => {
  ...
  m.push(...tableIndices.map(el => ({
    nodeId: IS_TESTING ? '_' + [...ip, 'c', ...el].join('') : 'node' + genHash(8),
    path: [...ip, 'c', ...el]
  } as N)))
  m.sort(sortPath)
}
