export enum MRT {
  undo = 'undo',
  redo = 'redo',
  saveView = 'saveView',
  saveFromCoordinates = 'saveFromCoordinates',
  startEditReplace = 'startEditReplace',
  startEditAppend = 'startEditAppend',
  removeMapListEntriesOfEdit = 'removeMapListEntriesOfEdit',

  LOAD = 'LOAD',

  setDensitySmall = 'setDensitySmall',
  setDensityLarge = 'setDensityLarge',
  setPlaceTypeExploded = 'setPlaceTypeExploded',
  setPlaceTypeIndented = 'setPlaceTypeIndented',

  selectT = 'selectT',
  selectXR = 'selectXR',
  selectSelfX = 'selectSelfX',
  selectFamilyX = 'selectFamilyX',
  selectSD = 'selectSD',
  selectSU = 'selectSU',
  selectSO = 'selectSO',
  selectSI = 'selectSI',
  selectSF = 'selectSF',
  selectCFfirstRow = 'selectCFfirstRow',
  selectCFfirstCol = 'selectCFfirstCol',
  selectCFF = 'selectCFF',
  selectXSIC = 'selectXSIC',
  selectTtoo = 'selectTtoo',
  selectSDtoo = 'selectSDtoo',
  selectSUtoo = 'selectSUtoo',
  selectRA = 'selectRA',
  selectSA = 'selectSA',
  selectCRSAME = 'selectCRSAME',
  selectCCSAME = 'selectCCSAME',
  selectCD = 'selectCD',
  selectCU = 'selectCU',
  selectCR = 'selectCR',
  selectCL = 'selectCL',
  selectByRectanglePreview = 'selectByRectanglePreview',
  selectByRectangle = 'selectByRectangle',

  insertL = 'insertL',
  insertR = 'insertR',
  insertSD = 'insertSD',
  insertSU = 'insertSU',
  insertSO = 'insertSO',
  insertSOText = 'insertSOText',
  insertSOLink = 'insertSOLink',
  insertSOImage = 'insertSOImage',
  insertCRD = 'insertCRD',
  insertCRU = 'insertCRU',
  insertSCRD = 'insertSCRD',
  insertSCRU = 'insertSCRU',
  insertCCR = 'insertCCR',
  insertCCL = 'insertCCL',
  insertSCCR = 'insertSCCR',
  insertSCCL = 'insertSCCL',
  insertSOTable = 'insertSOTable',

  gptParseNodesS = 'gptParseNodesS',
  gptParseNodesT = 'gptParseNodesT',
  gptParseNodeMermaid = 'gptParseNodeMermaid',

  deleteL = 'deleteL',
  deleteLR = 'deleteLR',
  deleteS = 'deleteS',
  deleteCR = 'deleteCR',
  deleteCC = 'deleteCC',

  cutLR = 'cutLR',
  cutS = 'cutS',
  copyLR = 'copyLR',
  copyS = 'copyS',
  pasteLR = 'pasteLR',
  pasteSO = 'pasteSO',
  duplicateR = 'duplicateR',
  duplicateS = 'duplicateS',
  moveSD = 'moveSD',
  moveST = 'moveST',
  moveSU = 'moveSU',
  moveSB = 'moveSB',
  moveSO = 'moveSO',
  moveSI = 'moveSI',
  moveByDragPreview = 'moveByDragPreview',
  moveByDrag = 'moveByDrag',
  moveCRD = 'moveCRD',
  moveCRU = 'moveCRU',
  moveCCR = 'moveCCR',
  moveCCL = 'moveCCL',
  moveS2TO = 'moveS2TO',
  transpose = 'transpose',

  setTaskStatus = 'setTaskStatus',
  setContentText = 'setContentText',
  setContentEquation = 'setContentEquation',
  setContentMermaid = 'setContentMermaid',
  setControlTypeNone = 'setControlTypeNone',
  setControlTypeIngestion = 'setControlTypeIngestion',
  setControlTypeExtraction = 'setControlTypeExtraction',
  offsetD = 'offsetD',
  offsetU = 'offsetU',
  offsetR = 'offsetR',
  offsetL = 'offsetL',
  setLlmData = 'setLlmData',
  setLineWidth = 'setLineWidth',
  setLineType = 'setLineType',
  setLineColor = 'setLineColor',
  setSBorderWidth = 'setSBorderWidth',
  setFBorderWidth = 'setFBorderWidth',
  setSBorderColor = 'setSBorderColor',
  setFBorderColor = 'setFBorderColor',
  setSFillColor = 'setSFillColor',
  setFFillColor = 'setFFillColor',
  setTextFontSize = 'setTextFontSize',
  setTextColor = 'setTextColor',
  setBlur = 'setBlur',
  setTaskModeOn = 'setTaskModeOn',
  setTaskModeOff = 'setTaskModeOff',
  setTaskModeReset = 'setTaskModeReset',

  clearDimensions = 'clearDimensions',
  clearLlmData = 'clearLlmData',
  clearLine = 'clearLine',
  clearSBorder = 'clearSBorder',
  clearFBorder = 'clearFBorder',
  clearSFill = 'clearSFill',
  clearFFill = 'clearFFill',
  clearText = 'clearText',
  clearBlur = 'clearBlur',
}
