...
describe("InsertTests", () => {
  ...
  test('insertSOR', () => {
    ...
    expect(mapDeInit(mapReducerAtomic(mapInit(test) as M, 'insertSOR', {}) as M).sort(sortNode)).toEqual((result as M).sort(sortNode))
  })
  test('insertSO', () => {
    const test = [
      {nodeId: 'a', path: ['g']},
  ...
  test('insertSORTable', () => {
    ...
    expect(mapDeInit(mapReducerAtomic(mapInit(test) as M, 'insertSORTable', {rowLen: 2, colLen: 2}) as M).sort(sortNode)).toEqual((result as M).sort(sortNode))
  })
})
