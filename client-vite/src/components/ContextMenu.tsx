          ...
          <li>
            ...
            <div id="editSubMenu" className={subMenuClassName}>
              <ul className="py-2 text-sm text-gray-700 dark:text-gray-200" aria-labelledby="doubleDropdownButton">
                ...
                { mExists && getXAEO(m).map(t => t.taskStatus).includes(0) && <li><a className={menuClassName} onClick={()=>{dispatch(actions.mapAction({type: 'setTaskModeOn', payload: null}))}}>Task Mode On</a></li> }
                { mExists && getXAEO(m).map(t => t.taskStatus).some(el => el > 0) && <li><a className={menuClassName} onClick={()=>{dispatch(actions.mapAction({type: 'setTaskModeOff', payload: null}))}}>Task Mode Off</a></li> }
                { mExists && getXAEO(m).map(t => t.taskStatus).some(el => el > 0) && <li><a className={menuClassName} onClick={()=>{dispatch(actions.mapAction({type: 'setTaskModeReset', payload: null}))}}>Task Mode Reset</a></li> }
