...
import {getCountXASD, getCountXASU, getMapMode, getX, getXAEO, isXASVN, isXRS, isXS} from "../../queries/MapQueries.ts"
    ...
    <DropdownMenu.Root>
      ...
      {mapMode === MapMode.EDIT_STRUCT && isXS(m) &&
        <DropdownMenu.Content>
          ...
          {getX(m).so1.length > 0 && !getXAEO(m).some(ti => ti.path.includes('c')) && <DropdownMenu.Item onClick={() => md(MR.moveS2TO)}>{'Sub Nodes To Table'}</DropdownMenu.Item>}
        </DropdownMenu.Content>
      }
    </DropdownMenu.Root>
  )
