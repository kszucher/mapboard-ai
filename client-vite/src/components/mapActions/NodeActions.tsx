      ...
      {nodeMode === NodeMode.EDIT_STRUCT &&
        <DropdownMenu.Content onCloseAutoFocus={e => e.preventDefault()}>
          ...
          <DropdownMenu.Sub>
            <DropdownMenu.SubTrigger>{'Move'}</DropdownMenu.SubTrigger>
            <DropdownMenu.SubContent>
              {isXASVN(m) && getXFS(m).su.length > 0 && <DropdownMenu.Item onClick={() => dm(MM.moveSU)}>{'Node Up'}</DropdownMenu.Item>}
              {isXASVN(m) && getXLS(m).sd.length > 0 && <DropdownMenu.Item onClick={() => dm(MM.moveSD)}>{'Node Down'}</DropdownMenu.Item>}
              {isXASVN(m) && getXFS(m).su.length > 0 && <DropdownMenu.Item onClick={() => dm(MM.moveSO)}>{'Node Out'}</DropdownMenu.Item>}
              {!isXARS(m) && isXASVN(m) && <DropdownMenu.Item onClick={() => dm(MM.moveSI)}>{'Node In'}</DropdownMenu.Item>}
          ...
          <DropdownMenu.Sub>
            ...
            <DropdownMenu.SubContent>
              ...
              {getXAS(m).length ===  1 && getXS(m).co.length === 0 && [getXS(m).nodeId, ...getXS(m).so].map(nid => idToS(m, nid)).map(ti => ti.taskStatus).some(el => el > 0) && <DropdownMenu.Item onClick={() => dm(MM.setTaskModeOff)}>{'Task Mode Off'}</DropdownMenu.Item>}
              {getXAS(m).length ===  1 && getXS(m).co.length === 0 && [getXS(m).nodeId, ...getXS(m).so].map(nid => idToS(m, nid)).map(ti => ti.taskStatus).some(el => el > 0) && <DropdownMenu.Item onClick={() => dm(MM.setTaskModeReset)}>{'Task Mode Reset'}</DropdownMenu.Item>}
