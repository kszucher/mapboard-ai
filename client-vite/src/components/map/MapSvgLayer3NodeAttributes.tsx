...
import {getTSI1, getTSI2, getCountTCO1, getCountTSO1, getG, getNodeById, getPathPattern, isR, isS, mT} from "../../selectors/MapSelector"
    ...
    <g>
      {mT(m).map(ti => (
        <Fragment key={ti.nodeId}>
          ...
          {(getPathPattern(ti.path).endsWith('rs') && getCountTCO1(m, ti) === 0 || (getPathPattern(ti.path).endsWith('ss') && getCountTCO1(m, ti) === 0)) &&
            <path
          ...
          {((getPathPattern(ti.path).endsWith('rsc') || getPathPattern(ti.path).endsWith('ssc')) && ti.path.at(-2) > -1 && ti.path.at(-1) === 0) &&
            <path
          ...
          {
            ti.taskStatus > 0 && !isR(ti.path) && getCountTSO1(m, ti) === 0 && getCountTCO1(m, ti) === 0 && ti.contentType !== 'image' &&
            <Fragment key={`${ti.nodeId}_svg_task`}>
              {
                !isEqual(ti.nodeId, editedNodeId) &&
                <path
                  d={getLinearLinePath({x1: adjust(ti.nodeEndX), x2: adjust(getTaskStartPoint(m, g, ti)), y: adjust(ti.nodeY)})}
                  stroke={C.TASK_LINE}
                  strokeWidth={1}
                  fill={'none'}
                  {...pathCommonProps}
