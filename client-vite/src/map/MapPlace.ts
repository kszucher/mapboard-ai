import {getNodeByPath, getParentPath, isG, isR, isD, isS, isC, is_S_U_siblingPath, getPattern} from "./MapUtils"
  ...
  m.forEach(n => {
    if (isG(n.path)) {
      ...
      if (isS(n.path)) {
        ...
        const sumUpperSiblingMaxH = m.filter(nt => is_S_U_siblingPath(n.path, nt.path)).map(n => n.maxH).reduce((a, b) => a + b, 0)
