...
export const createPathByIncrementAt = (p: Path, at: number) => structuredClone(p.map((pi, i) => i === at ? pi + 1 : pi))
export const createPathByDecrementAt = (p: Path, at: number) => structuredClone(p.map((pi, i) => i === at ? pi - 1 : pi))
...
export const fFinder = (m: M) => m.filter((n: N) => getG(m).sc.structSelectedPathList.some((sp: Path) => isEqual(sp, n.path) || isDescendantPath(sp, n.path) && isS(n.path)))
