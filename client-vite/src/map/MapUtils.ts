...
export const incSSODO = (m: M) => m.filter(n => isSSODO(getLS(m).path, n.path)).forEach(n => n.path = inc_pi(n.path, getLS(m).path.length - 1))
export const incSDO = (m: M) => m.filter(n => isSDO(getLS(m).path, n.path)).forEach(n => n.path = inc_pi(n.path, getLS(m).path.length - 1))
export const incGtCR = (m: M) => m.filter(n => isGtCR(getLS(m).path, n.path)).forEach(n => n.path = inc_pi(n.path, getLS(m).path.length - 1))
export const incGteCR = (m: M) => m.filter(n => isGteCR(getLS(m).path, n.path)).forEach(n => n.path = inc_pi(n.path, getLS(m).path.length - 1))
export const incGtCD = (m: M) => m.filter(n => isGtCD(getLS(m).path, n.path)).forEach(n => n.path = inc_pi(n.path, getLS(m).path.length - 2))
export const incGteCD = (m: M) => m.filter(n => isGteCD(getLS(m).path, n.path)).forEach(n => n.path = inc_pi(n.path, getLS(m).path.length - 2))
