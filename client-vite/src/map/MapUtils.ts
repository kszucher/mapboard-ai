...
export const getSelectionFamily = (m: M) => m.filter(n => getSelection(m).map(n => n.path).some(p => is_S_F(p, n.path)))
...
export const is_S_U_F = (p: P, pt: P) => pt.length >= p.length && isEqual(pt.slice(0, p.length - 1), p.slice(0, -1)) && pt.at(p.length - 1) < p.at(-1)
export const is_S_D_F = (p: P, pt: P) => pt.length >= p.length && isEqual(pt.slice(0, p.length - 1), p.slice(0, -1)) && pt.at(p.length - 1) > p.at(-1)
export const is_S_O_D_F = (p: P, pt: P) => isEqual(p, pt) || is_S_O(p, pt) || is_S_D_F(p, pt)
