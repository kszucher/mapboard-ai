...
import {getCountNSO1, getCountXASD, getCountXASU, getCountXCO1, getCountXRXD0S, getCountXSO1, getCountXSO2, getRi, getRXD0, getRXD1, getX, getXP, isXASVN, isXR, isXS} from "../core/MapUtils"
        ...
        { mExists && (isXR(m) || getCountXSO1(m) > 0) &&
          <li>
            <a className={menuClassName} onClick={()=>{
              ...
              if (!isXS(m) && getCountXRXD0S(m) > 0 && !getRXD0(m, getRi(getXP(m))).selected) {dispatch(actions.mapAction({type: 'selectRXD0F', payload: {path: getXP(m)}}))
              } else if (!isXS(m) && !!getRXD0(m, getRi(getXP(m))).selected && !getRXD1(m, getRi(getXP(m))).selected && getCountNSO1(m, getRXD1(m, getRi(getXP(m)))) > 0) {dispatch(actions.mapAction({type: 'selectRXD1F', payload: {path: getXP(m)}}))
              } else if (!isXR(m) && getCountXSO1(m) > 0 && getX(m).selection === 'f') {dispatch(actions.mapAction({type: 'selectS', payload: {path: getXP(m)}}))
              } else if  (!isXR(m) && getCountXSO1(m) > 0 && getX(m).selection === 's') {dispatch(actions.mapAction({type: 'selectF', payload: {path: getXP(m)}}))}
            }}>Selection Change
            </a>
          </li>
        }
