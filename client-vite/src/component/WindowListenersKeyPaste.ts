...
export const windowListenersKeyPaste = (
  ...
  const stateMachine = [
    ...
    [ kd, ckm(e, '000') && code === 'Backspace',           c,           1, 'select_S_B',               {},                              1 ],
    [ kd, ckm(e, '000') && code === 'Backspace',           c,           1, 'select_S_BB',              {},                              1 ],
    [ kd, ckm(e, '000') && code === 'Escape',              s || c,      1, 'select_R',                 {},                              1 ],
    [ kd, ckm(e, '100') && code === 'KeyA',                true,        1, 'select_all',               {},                              1 ],
    ...
    [ kd, ckm(e, '100') && code === 'KeyZ',                true,        0, 'redo',                     {},                              1 ],
    [ kd, ckm(e, '100') && code === 'KeyY',                true,        0, 'undo',                     {},                              1 ],
    [ kd, ckm(e, '100') && code === 'KeyE',                s,           1, 'transpose',                {},                              1 ],
    [ kd, ckm(e, '000') && [L,R,U,D].includes(which),      r || s,      1, 'select_S_IOUD',            {...c2dt(m, which), add: false}, 1 ],
  ...
  for (let i = 0; i < stateMachine.length; i++) {
    const [ eventTypeCondition, match, scopeMatch, isMapAction, type, payload, preventDefault ] = stateMachine[i]
    if (eventTypeCondition && match === true && scopeMatch === true) {
