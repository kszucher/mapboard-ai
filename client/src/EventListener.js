...
class EventListener {
    ...
    keyLutInit() {
        let keyLutFields = Object.keys(this.keyLut);
        for (let i = 0; i < keyLutFields.length; i++) {
            this.keyLut[keyLutFields[i]].status = false;
        }
    }
    ...
    getKey(keyCode) {
        ...
        let keyLutFields = Object.keys(this.keyLut);
        for (let i = 0; i < keyLutFields.length; i++) {
            let currKey = this.keyLut[keyLutFields[i]];
    ...
    keydown(event) {
        ...
        let keyStr = this.getKey(keyCode);

        if (keyStr === 'VK_CONTROL') {
            this.controlStatus = true;
        }

        if(this.keyLut[keyStr] !== undefined) {
            this.keyLut[keyStr].status = true;
        }

        if(this.keyLut.VK_SHIFT.status === true) {
            let keyLutFields = Object.keys(this.keyLut);
            for (let i = 0; i < keyLutFields.length; i++) {
                let currKey = this.keyLut[keyLutFields[i]];
        ...
        else if(this.keyLut.VK_CONTROL.status === true) {
            let keyLutFields = Object.keys(this.keyLut);
            for (let i = 0; i < keyLutFields.length; i++) {
                let currKey = this.keyLut[keyLutFields[i]];
        ...
        else if(this.keyLut.VK_ALT.status === true) {
            let keyLutFields = Object.keys(this.keyLut);
            for (let i = 0; i < keyLutFields.length; i++) {
                let currKey = this.keyLut[keyLutFields[i]];
    ...
    keyup(event) {
        ...
        if(this.keyLut[keyStr] !== undefined) {
            this.keyLut[keyStr].status = false;
        }
        else {
